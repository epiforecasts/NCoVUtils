...
# NB. PDF table is not produced consistently each day, so code (lines 18 & 20) may need adjusting
#
...
get_spain_regional_cases <- function() {
  ...
  out <- tabulizer::extract_tables(location, pages = 1)
  cases <- do.call(rbind, out[length(out)])
  cases <- as.data.frame(cases[2:(nrow(cases)-1), ])
  cases <- cases %>%
    dplyr::select("region" = V1, "cases" = V8) %>%
    dplyr::mutate(region = iconv(region, from = 'UTF-8', to = 'ASCII//TRANSLIT'),
                  cases = as.character(cases),
                  cases = str_remove_all(cases, "[[:punct:]]"),
